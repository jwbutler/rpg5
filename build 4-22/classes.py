...
class Animation:
    ...
    def get_current_frame(self):
        ...
        return self.frames[self.findex-1]

    def get_current_filename(self):
        """ Returns the current frame # of the current animation. """
        return self.filenames[self.findex-1]
...
class Corpse:
  ...
  def get_rect(self, game):
    (left,top) = grid_to_pixel(game, self.x, self.y)
    top -= 28
...
class Wall:
  def __init__(self, x, y, icon, height_offset = 0):
    self.x = x
    self.y = y
  ...
  def draw(self, game):
    ...
    for unit in game.units + game.corpses:
      #if unit.playable:
    ...
    for unit in game.units + game.corpses:
      if unit.playable:
        if distance((unit.x,unit.y),(self.x,self.y)) <= game.vision_radius:
          game.screen.blit(self.icon, self.get_rect(game))
          self.icon.set_alpha(255)
          return
  ...
  def unit_to_alpha(self, game, unit):
    collide_rect = self.get_rect(game).clip(unit.get_rect(game))
    collide_area = collide_rect.width * collide_rect.height
    x = max(255-(collide_area), 0)
    return x
  
  def coords_to_alpha(self, game, x, y):
...
class Rocks(WallNoTrans):
  ...
  def remove_one(self, game):
    self.count -= 1
    if self.count == 0:
      game.rock_piles.remove(self)
    else:
      self.icon = self.icons[self.count]
...
class Parapet(WallNoTrans):
  ...
  def get_z(self):
    ...
    return 6*self.x + 6*self.y + 12*self.height_offset + 1
...
class Roof(Wall):
  ...
  def draw(self, game):
    ...
    for unit in game.units:
      if unit.get_z() < self.get_z():
        alpha = self.unit_to_alpha(game, unit)
        if alpha < img.get_alpha():
          self.icon.set_alpha(alpha)
    (x,y) = grid_to_pixel(game, self.rect.left, self.rect.top)
    if (self.rect.width, self.rect.height) == (3,3):
      ...
      y += -27*self.height_offset - 28
    game.screen.blit(img, (x,y))
    #game.draw_black_roof(self, img, (x,y))
...
class Gate(Wall):
  ...
  def draw(self, game):
    ...
    for unit in game.units:
      ...
      alpha = self.coords_to_alpha(game, unit.x, unit.y)
      if alpha < img.get_alpha():
        self.icon.set_alpha(alpha)
    (x,y) = grid_to_pixel(game, self.x_1, self.y_1)
    y -= 84
    x -= 3
    game.screen.blit(img, (x,y))
  ...
  def get_rect(self, game):
    (left,top) = grid_to_pixel(game, self.x_1, self.y_1)
...
class Door:
  ...
  def draw(self, game):
    ...
    if self.show_open_icon:
      ...
      source = self.icon
    if self.dir == 'E':
      game.screen.blit(source, grid_to_pixel(game, self.x, self.y))
    elif self.dir == 'S':
      game.screen.blit(source, grid_to_pixel(game, self.x, self.y))
...
class Floor:  
  def __init__(self, icons=[], rect=pygame.Rect((0,0,0,0))):
    ...
    for y in range(self.rect.height):
      for x in range(self.rect.width):
        self.tiles[(x,y)] = random.choice(self.icons)

  def append_rect(self, type, rect):
    icons = []
    i = 1
    while os.path.exists(type + '_' + str(i) + '.tga'):
      surface = pygame.image.load(type + '_' + str(i) + '.tga')
  ...
  def draw(self, game):
    rect = game.camera.get_rect()
    screen_rect = pygame.Rect(0,0,320,240)
    n = 0
    game.floor_layer.fill((0,0,0))
    for (x,y) in self.tiles.keys():
      if rect.collidepoint((x,y)):
        for unit in game.units:
          if unit.playable:
            if distance((unit.x,unit.y), (x,y)) <= game.vision_radius:
              (xx,yy) = grid_to_pixel(game, x, y)
              tile_rect = pygame.rect.Rect((xx,yy,24,12))
              if screen_rect.colliderect(tile_rect):
                game.floor_layer.blit(self.tiles[(x,y)], (xx, yy))
                break
...
class BasicObject(Wall):
  ...
  def draw(self, game):
    ...
    game.screen.blit(self.icon, (x,y))
  

class Menu:
    def __init__(self, image, rect, player, visible = False):
        self.image = image
        self.rect = rect
...
class MainMenu(Menu):
  def __init__(self, image, rect, visible = False):
    self.image = image
    self.rect = rect
    self.background = pygame.image.load(self.image)
  ...
  def refresh(self, game):
    ...
    for unit in game.units:
      if unit.playable:
        self.cards.append(UnitCard(game, unit))
...
#check functions(2) for these
def wall_box(x1, y1, x2, y2, textures, height_offset = 0):
  """ empty in the middle """
  walls = []
    ...
    for y in range(y1, y2+1):
      for x in range(x1, x2+1):
        if (x == x1) or (x == x2) or (y == y1) or (y == y2):
          ...
          walls.append(Wall(x, y, texture, z))
  return walls
  
def parapet_box(x1, y1, x2, y2, direction, height_offset = 0):
  """ empty in the middle """
  parapets = []
  for y in range(y1, y2+1):
    for x in range(x1, x2+1):
...
class Blood:
  def __init__(self, x, y, damage, (min_r,min_g,min_b)=(128,0,0), (max_r,max_g,max_b)=(224,32,24)):
    (self.x, self.y) = (x,y)  
...
class Dart:
  def refresh(self, game):
    ...
    if target_unit:
      ...
      self.direction = coords_to_dir((dx, dy))
      if self.direction == '':
        del(self)
  ...
  def __init__(self, game, damage, source_unit, target_unit):
    ...
    self.speed = 2
    self.damage = damage
    (self.x_i, self.y_i) = (game.unit_by_name(self.source_unit).x, game.unit_by_name(self.source_unit).y)
    (self.x, self.y) = (self.x_i, self.y_i)
    self.refresh(game)
    self.do_events(game) #this way it'll move once first
  
  def do_events(self, game):
    if not game.unit_by_name(self.target_unit):
      try:
        game.darts.remove(self)
      except:
    ...
    self.x += self.dx
    self.y += self.dy
    source_unit = game.unit_by_name(self.source_unit)
    target_unit = game.unit_by_name(self.target_unit)
    if distance((self.x, self.y), (target_unit.x, target_unit.y)) <= self.speed:
      target_unit.take_damage(game, self.damage)
      game.blood.append(Blood(target_unit.x, target_unit.y, self.damage))
  ...
  def draw(self, game):
    ...
    (x,y) = grid_to_pixel(game, self.x, self.y)
    y -= 20
    remaining_distance = distance((self.x, self.y), (target_unit.x, target_unit.y))
    total_distance = distance((source_unit.x, source_unit.y), (target_unit.x, target_unit.y))
    ...
    y -= height_offset
    game.screen.blit(self.current_icon, (x,y))
...
class WaterTile:
  ...
  def draw(self, game):
    game.screen.blit(self.get_current_frame(), grid_to_pixel(game, self.x, self.y))
    #game.draw_black(self, self.get_current_frame(), grid_to_pixel(game, self.x, self.y))
...
class FireTile(WaterTile):
  ...
  def draw(self, game):
    (x,y) = grid_to_pixel(game, self.x, self.y)
    y -= 18
    game.screen.blit(self.get_current_frame(), (x,y))
    #game.draw_black(self, self.get_current_frame(), (x,y))
...
class Fireball(Dart):
  ...
  def do_events(self, game):
    ...
    if self.impact:
      ...
      self.x += self.dx
      self.y += self.dy
      if distance((self.x, self.y), (target_unit.x, target_unit.y)) <= 1:
        (self.x, self.y) = (target_unit.x, target_unit.y)
        dmg = source_unit.damage()
        target_unit.take_damage(game, dmg)
        game.blood.append(Blood(target_unit.x, target_unit.y, dmg))
        if target_unit.current_hp <= 0 and target_unit.current_activity not in ['falling', 'dead', 'decapitated', 'dead_decapitated']:
          target_unit.refresh_activity(game, 'falling')
          target_unit.reset_ticks()
          target_unit.scream()
  ...
  def draw(self, game):
    ...
    if self.impact:
      y += 10
      game.screen.blit(self.impact_frames[self.impact_findex], (x,y))
      self.impact_findex += 1
      if self.impact_findex == 3:
        self.burning = True
        self.impact = False
    elif self.burning:
      game.screen.blit(self.burning_frames[self.burning_findex], (x,y))
      self.burning_findex += 1
      if self.burning_findex == 4:
        game.darts.remove(self)        
    else:
      y += 10
      game.screen.blit(self.frames[self.findex], (x,y))
      self.findex += 1
      if self.findex > 3:
        self.findex = 0
...
class LaserBeam(Dart):
  ...
  def do_events(self, game):
    if self.ticks >= 16:
      ...
      if not obstacle_unit(game, (self.x+self.dx,self.y+self.dy)):
        game.darts.remove(self)
        return True
    self.x += self.dx
    self.y += self.dy
    for unit in game.units:
      if (unit.x,unit.y) == (self.x,self.y):
        source_unit = game.unit_by_name(self.source_unit)
        if source_unit:
          if unit.name != source_unit.name:
            unit.take_damage(game, 20)
            game.blood.append(Blood(unit.x, unit.y, 20))
...
class HealingFlash:
  def __init__(self, game, unit_name):
    ...
    unit = game.unit_by_name(self.unit_name)
  ...
  def draw(self, game):
    unit = game.unit_by_name(self.unit_name)
    (self.x,self.y) = (unit.x,unit.y)
    (x,y) = grid_to_pixel(game, unit.x, unit.y)
    x -= 8; y -= 30    
    game.screen.blit(self.frames[self.findex], (x,y))
    if not game.paused:
      self.findex += 1
      if self.findex >= len(self.frames):
        game.healing_flashes.remove(self)
...
class Pointer:
  ...
  def return_unit(self, game):
    return game.unit_by_name(self.unit_name)
...
class HealthPowerup(Wall):
  def __init__(self, (x,y), heal_amount):
    ...
    for (src_color, dest_color) in palette_swaps.iteritems():
      self.icon = palette_swap(self.icon, src_color, dest_color)
    (self.x,self.y) = (x,y)
    self.heal_amount = heal_amount
    self.height_offset = 0
    
  def proc(self, game, unit):
