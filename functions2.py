...
def rgb_to_invisible_walls((r,g,b), (x,y)):
...
def pixel_array_to_bandit_generators(game, pixel_array, super_frequency):
  ...
  for y in range(pixel_array.surface.get_height()):
    for x in range(pixel_array.surface.get_width()):
      (r,g,b,a) = pixel_array.surface.unmap_rgb(pixel_array[x][y])
      if (r,g,b) == (255,128,64):
        generators.append(BanditGenerator(game, (x,y), 20, super_frequency))
  return generators
  
def pixel_array_to_infinite_bandit_generators(game, pixel_array, super_frequency):
  generators = []
  for y in range(pixel_array.surface.get_height()):
    for x in range(pixel_array.surface.get_width()):
      (r,g,b,a) = pixel_array.surface.unmap_rgb(pixel_array[x][y])
      if (r,g,b) == (255,128,64):
        generators.append(InfiniteBanditGenerator(game, (x,y), super_frequency, 6, 0.008))
  return generators
...
def pixel_array_to_peasants(game, pixel_array):
  # radius should be an argument!
  units = []
  for y in range(pixel_array.surface.get_height()):
    for x in range(pixel_array.surface.get_width()):
      ...
      if (r,g,b) == (128,0,255):
        ...
        units.append(RockThrowingPeasant(game, name, (x,y), None))
  return units
