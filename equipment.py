...
class Armor:
  """ body armor """
  def __init__(self, game, name, anim_name, activities, palette_swaps = {}):
    self.game = game
    self.name = name
    ...
    self.icon = self.game.palette_swap_multi(self.icon, self.palette_swaps)
    self.icon_small = self.game.palette_swap_multi(self.icon_small, self.palette_swaps)
    self.animations = []
  ...
  def set_animation(self, activity, directions):
      ...
      for dir in self.game.directions:
        ...
        for frame in frames:
          try:
            ...
            try:
              surface = self.game.sprite_cache[frame].copy() #Copy is necessary
...
class Helm(Armor):
  def __init__(self, game, name, anim_name, activities, palette_swaps = {}):
    Armor.__init__(self, game, name, anim_name, activities, palette_swaps)
    self.slot = "head"
...
class Hairpiece(Armor):
  ...
  def __init__(self, game, name, anim_name, activities, palette_swaps = None):
    self.name = name
    self.game = game
    self.anim_name = anim_name
    self.slot = "hair"
...
class Beard(Hairpiece):
  def __init__(self, game, name, activities, palette_swaps):
    Hairpiece.__init__(self, game, name, 'beard', activities, palette_swaps)
    self.slot = 'beard'
    ...
    for activity in self.activities:
      for dir in self.game.directions:
        ...
        if activity == 'standing':
          ...
          for x in ['attacking_'+dir+'_1','attacking_'+dir+'_1','attacking_'+dir+'_2','attacking_'+dir+'_2',
                    'attacking_'+dir+'_2','attacking_'+dir+'_2','attacking_'+dir+'_1','standing_'+dir+'_1']:
...
class Sword:
    """
    This class is also used for clubs.
    """
    def __init__(self, game, name, anim_name, activities, palette_swaps={}):
      t1 = pygame.time.get_ticks()
      self.game = game
        ...
        for dir in self.game.directions:
          ...
          if activity == 'walking':
            ...
            if dir in ['S', 'SW', 'W']:
              ...
              anim_dir = 'NE'
            if os.path.exists('tga/'+self.anim_name+'_falling_S_1.tga'):
              for x in [1,2,3,4,4]: #KLUDGE
                for n in range(2):
                  frames.append('tga/'+self.anim_name+'_falling_'+anim_dir+'_'+str(x)+'.tga')
            elif os.path.exists('tga/'+self.anim_name+'_falling_1.tga'): #Club
              for x in [1,1,2,2,2]: #KLUDGE
                for n in range(2):
                  frames.append('tga/'+self.anim_name+'_falling_'+str(x)+'.tga')
            ...
            if dir in ['S', 'SW', 'W']:
              ...
              anim_dir = 'NE'
            if os.path.exists('tga/'+self.anim_name+'_falling_S_4.tga'):              
              frames.append('tga/'+self.anim_name+'_falling_'+anim_dir+'_4.tga')
            elif os.path.exists('tga/'+self.anim_name+'_falling_1.tga'):
              frames.append('tga/'+self.anim_name+'_falling_2.tga')
            ...
            for frame in frames:
              try:
                surface = frames_surfaces[frame].copy() #copy() IS necessary here
              except KeyError:
                try:
                  surface = self.game.sprite_cache[frame].copy()
          ...
          if activity not in ['healing', 'slashing']:
            ...
            for frame in frames:
              ...
              frame_surfaces.append(surface)
            self.animations.append(Animation(activity, [dir], frame_surfaces, frames))
    def draw(self, x, y, black_alpha=0):
      source = self.get_current_frame()
      if source:
...
class FireSword(Sword):
  def __init__(self, game, name, activities, anim_name):
    self.game = game
    ...
    self.activities = activities
    self.current_activity = None
    self.load_animations(self.game, {})
    self.selected_in_menu = False
  ...
  def draw(self, x, y):
    ...
    source = self.get_current_frame()
    if source:
      s2 = self.game.palette_swap(source, ps)
      s2.set_colorkey((255,255,255))
      self.game.screen.blit(s2, (x, y))

# # # # # # # # # # # #
# Equipment instances #
# # # # # # # # # # # #

#This is basically a giant lookup table
def make(game, name, female=False):
  # Try to transition out of this
  if name == 'sword_of_suck':
    ...
    if female:
      return Sword(game, "Steel Sword", "sword_female", {(128,128,128): (189,191,193), (192,192,192): (234,229,229)})
    else:
    ...
    return Shield(game, "Green Bandit Shield", "shield2", {(128,128,128):(128,128,0), (192,192,192):(170,190,90), (0,0,0):(100,140,100)})
